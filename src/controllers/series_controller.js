...
export const SeriesController = {
  getAll: async (req, res) => {
    try {
      ...
      return httpResponse.NOT_FOUND(res, err);
    }
  },
  ...
  updateFileById: async (req, res) => {
    try {
      const file = await SeriesService.update(req.params.id, req.body, {
        new: true,
        runValidators: true,
      });
      if (!file) {
  ...
  getAllSeasonsOfSeriesBySeriesId: async (req, res) => {
    try {
      const data = await SeriesService.getAllSeasonsOfSeriesBySeriesId(
        req.params.id
      );
      return httpResponse.SUCCESS(res, data);
    } catch (err) {
  ...
  getAllEpisodesOfSeriesById: async (req, res) => {
    try {
      const data = await SeriesService.getAllEpisodesOfSeriesById(
        req.params.id
      );
      return httpResponse.SUCCESS(res, data);
    } catch (err) {
